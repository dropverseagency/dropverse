...
export default function Home() {
  ...
  return <main className="overflow-hidden">
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-[#071f1d]/80 backdrop-blur-xl">
      <div className="container flex h-20 items-center justify-between">
        ...
        <div className="hidden items-center gap-3 md:flex"><a href="#login" className="px-4 py-2 text-sm text-[#d9e0dc]">Login</a><a href="#start" className="rounded-full bg-[#d8b45a] px-5 py-2.5 text-sm font-bold text-[#10221f] transition hover:bg-[#f0d98b]">Get Started</a></div>
        <button onClick={()=>setMenu(!menu)} className="md:hidden" aria-label="Menu">{menu?<X/>:<Menu/>}</button>
      </div>
      {menu&&<div className="border-t border-white/5 bg-[#071f1d] p-5 md:hidden"><div className="container flex flex-col gap-5 text-[#d9e0dc]"><a href="#services" onClick={()=>setMenu(false)}>Services</a><a href="#how" onClick={()=>setMenu(false)}>How It Works</a><a href="#samples" onClick={()=>setMenu(false)}>Work Samples</a><a href="#about" onClick={()=>setMenu(false)}>About</a><a href="#start" className="text-[#d8b45a]">Get Started →</a></div></div>}
    </header>

    <section className="grid-bg relative flex min-h-screen items-center pt-20">
      <div className="absolute left-1/2 top-1/3 h-[420px] w-[420px] -translate-x-1/2 rounded-full bg-[#d8b45a]/5 blur-[100px]"/>
      <div className="container relative grid items-center gap-14 py-24 lg:grid-cols-[1.05fr_.95fr]">
        <div>
          <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-[#d8b45a]/20 bg-[#d8b45a]/5 px-4 py-2 text-xs font-semibold uppercase tracking-[.2em] text-[#e4c979]"><Sparkles size={14}/> Linking talent to sales</div>
          ...
          <div className="mt-9 flex flex-wrap gap-3"><a href="#start" className="group flex items-center gap-3 rounded-full bg-[#d8b45a] px-6 py-3.5 font-bold text-[#10221f] transition hover:bg-[#f0d98b]">Start Your Journey <ArrowRight size={18} className="transition group-hover:translate-x-1"/></a><a href="#services" className="flex items-center gap-2 rounded-full border border-white/10 px-6 py-3.5 font-semibold text-white transition hover:border-[#d8b45a]/40">Explore Services <ChevronRight size={18}/></a></div>
        ...
        <div className="relative mx-auto w-full max-w-[500px]">
          <div className="card glow relative overflow-hidden rounded-[28px] p-5">
            <div className="mb-5 flex items-center justify-between border-b border-white/10 pb-4"><div><div className="text-xs uppercase tracking-[.18em] text-[#718781]">DropVerse platform</div><div className="mt-1 font-display font-bold">Your service engine</div></div><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#d8b45a]/10 text-[#d8b45a]"><Zap size={17}/></div></div>
            <div className="space-y-3">
              {[['Talent','Skilled freelancers','01'],['Service','Ready-to-sell offers','02'],['Client','Your next opportunity','03'],['Sale','Revenue generated','04']].map(([a,b,n],i)=><div key={a} className="flex items-center gap-4 rounded-2xl border border-white/5 bg-white/[.025] p-4"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#d8b45a]/10 text-xs font-bold text-[#d8b45a]">{n}</div><div className="flex-1"><div className="font-semibold">{a}</div><div className="text-sm text-[#81948e]">{b}</div></div>{i<3&&<ArrowRight size={16} className="text-[#536963]"/>}{i===3&&<Sparkles size={16} className="text-[#d8b45a]"/>}</div>)}
            </div>
            <div className="mt-5 rounded-2xl border border-[#d8b45a]/15 bg-[#d8b45a]/5 p-4"><div className="flex items-center justify-between text-xs text-[#9aaba6]"><span>Business momentum</span><span className="text-[#d8b45a]">Growing</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-white/5"><div className="h-full w-[78%] rounded-full bg-[#d8b45a]"/></div></div>
          </div>
        </div>
      </div>
    </section>
